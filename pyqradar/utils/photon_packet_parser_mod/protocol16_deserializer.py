...
from pyqradar.utils.photon_packet_parser_mod.operation_response import OperationResponse
...
class Protocol16Deserializer:
    ...
    def deserialize_event_data(input: io.BytesIO):
        ...
        if code == 3:
            bytes = bytearray(parameters[1])

            position0 = struct.unpack_from('<f', bytes, 9)[0]
            position1 = struct.unpack_from('<f', bytes, 13)[0]
